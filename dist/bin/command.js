...
_commander2.default.version('1.0.0').usage('test').option('-t, --token [token]', 'Slack Access Token').option('-c, --channel [channel id]', 'Slack Channel ID to retrieve data').option('-f, --from [from timestamp in seconds]', 'Timestamp in seconds from when to get history[defaults to today]').parse(process.argv);
